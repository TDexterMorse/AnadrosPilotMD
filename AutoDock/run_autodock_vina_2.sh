...
# Find the PDBQT files in the current directory
pdbqt_files=$(ls *.pdbqt)

# Check if there are exactly two PDBQT files
file_count=$(echo "$pdbqt_files" | wc -l)
if [ $file_count -ne 2 ]; then
    echo "Error: There should be exactly two PDBQT files in the current directory."
    exit 1
fi

# Find the file with more lines (assumed to be the protein)
protein_file=$(echo "$pdbqt_files" | awk '{print length, $0}' | sort -n | awk '{print $2}' | head -n 1)
ligand_file=$(echo "$pdbqt_files" | grep -v "$protein_file")
...
docker pull biocontainers/autodock-vina
